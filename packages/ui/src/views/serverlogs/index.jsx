...
const searchTimeRanges = ['Last hour', 'Last 4 hours', 'Last 24 hours', 'Last 2 days', 'Last 7 days', 'Custom']
const searchTimeRangesForPro = [
    ...searchTimeRanges.slice(0, -1),
    'Last 14 days',
    'Last 1 month',
    'Last 2 months',
    'Last 3 months',
...
const Logs = () => {
    ...
    const [userPlan, setUserPlan] = useState('starter_monthly')
    const [accountLoading, setAccountLoading] = useState(true)

    const getSearchRanges = () => {
        if (accountLoading) return []
        else {
            if (userPlan === 'starter_monthly') return searchTimeRanges
            else return searchTimeRangesForPro
        }
    }
    ...
    const getMinDate = () => {
        if (userPlan === 'starter_monthly') return getDateBefore('days', 7)
        else if (userPlan === 'pro_monthly') return getDateBefore('months', 3)
    ...
    useEffect(() => {
        ...
        if (isCloud) getAccountDataApi.request()
    ...
    useEffect(() => {
        if (getAccountDataApi.data) {
            setUserPlan(getAccountDataApi.data?.org?.plan ?? 'starter_monthly')
        }
    }, [getAccountDataApi.data])

    useEffect(() => {
        setAccountLoading(getAccountDataApi.loading)
                    ...
                    {isLoading ? (
                        ...
                        <>
                            <Stack sx={{ alignItems: 'center', justifyContent: 'flex-start', gap: 2 }} flexDirection='row'>
                                ...
                                >
                                    {(isCloud ? getSearchRanges() : searchTimeRangesForPro).map((range) => (
