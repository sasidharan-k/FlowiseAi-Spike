...
const CreateEditRoleDialog = ({ show, dialogProps, onCancel, onConfirm, setError }) => {
            ...
            if (dialogData.permissions && dialogData.permissions.length > 0) {
                Object.keys(permissions).forEach((category) => {
                    Object.keys(permissions[category]).forEach((key) => {
                        dialogData.permissions.forEach((perm) => {
    ...
    const createRole = async () => {
        try {
            ...
            saveObj.permissions = tempPermissions
            let saveResp
            if (dialogProps.type === 'EDIT') {
                const id = dialogProps.data.id
                saveResp = await authApi.updateRole(id, saveObj)
            } else {
                saveResp = await authApi.createRole(saveObj)
            }
            if (saveResp.data) {
                enqueueSnackbar({
