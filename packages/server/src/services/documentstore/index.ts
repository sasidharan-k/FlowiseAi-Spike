...
import { databaseEntities, getAppVersion, getOrgData, saveUpsertFlowData } from '../../utils'
...
import { updateStorageUsage, checkStorage } from '../../utils/featureUsage'
import { DOCUMENT_STORE_BASE_FOLDER } from '../../utils/constants'
...
const deleteDocumentStore = async (storeId: string) => {
    try {
        ...
        const { totalSize } = await removeFilesFromStorage(orgData?.id, DOCUMENT_STORE_BASE_FOLDER, entity.id)
        await updateStorageUsage(orgData?.subdomain, totalSize / 1024 / 1024)
...
const _saveFileToStorage = async (fileBase64: string, entity: DocumentStore, orgData: ICommonObject) => {
    ...
    const { totalSize } = await addSingleFileToStorage(mime, bf, filename, orgData?.id, DOCUMENT_STORE_BASE_FOLDER, entity.id)
    await updateStorageUsage(orgData?.subdomain, totalSize / 1024 / 1024)
        ...
        if (input.startsWith('FILE-STORAGE::')) {
            ...
            if (currentLoader) {
                ...
                for (const file of files) {
                    const bf = await getFileFromStorage(file, orgData?.id, DOCUMENT_STORE_BASE_FOLDER, documentStoreEntity.id)
...
const previewChunksMiddleware = async (data: IDocumentStoreLoaderForPreview, orgData: ICommonObject) => {
    try {
        const appServer = getRunningExpressApp()
        ...
        if (process.env.MODE === MODE.QUEUE) {
            ...
            logger.debug(`[server]: [${orgData?.id}]: Job added to queue: ${job.id}`)
...
export const previewChunks = async ({ appDataSource, componentNodes, data, orgData }: IExecutePreviewLoader) => {
    try {
        if (data.preview) {
            if (
        ...
        if (!data.rehydrated) {
            await _normalizeFilePaths(appDataSource, data, null, orgData)
        }
...
export const processLoader = async ({ appDataSource, componentNodes, data, docLoaderId, orgData }: IExecuteProcessLoader) => {
    const entity = await appDataSource.getRepository(DocumentStore).findOneBy({
        id: data.storeId
    })
    if (!entity) {
    ...
    await _saveChunksToStorage(appDataSource, componentNodes, data, entity, docLoaderId, orgData)
    return getDocumentStoreFileChunks(appDataSource, data.storeId as string, docLoaderId)
}

const processLoaderMiddleware = async (data: IDocumentStoreLoaderForPreview, docLoaderId: string, orgData: ICommonObject) => {
    try {
        const appServer = getRunningExpressApp()
        const appDataSource = appServer.AppDataSource
        const componentNodes = appServer.nodesPool.componentNodes
...
const _saveChunksToStorage = async (
    ...
    try {
        ...
        await _normalizeFilePaths(appDataSource, data, entity, orgData)
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            if (input.startsWith('[') && input.endsWith(']')) {
                ...
                for (let j = 0; j < files.length; j++) {
                    const file = files[j]
                    if (re.test(file)) {
                        const fileMetadata = await _saveFileToStorage(file, entity, orgData)
                ...
                const fileNames: string[] = []
                const fileMetadata = await _saveFileToStorage(input, entity, orgData)
                fileNames.push(fileMetadata.name)
                filesWithMetadata.push(fileMetadata)
                data.loaderConfig[keys[i]] = 'FILE-STORAGE::' + JSON.stringify(fileNames)
                break
...
export const insertIntoVectorStore = async ({
    ...
    try {
        ...
        const indexResult = await _insertIntoVectorStoreWorkerThread(appDataSource, componentNodes, telemetry, data, isStrictSave, orgData)
        return indexResult
    } catch (error) {
        throw new InternalFlowiseError(
            StatusCodes.INTERNAL_SERVER_ERROR,
...
const insertIntoVectorStoreMiddleware = async (data: ICommonObject, isStrictSave = true, orgData: ICommonObject) => {
    try {
        const appServer = getRunningExpressApp()
        const appDataSource = appServer.AppDataSource
        const componentNodes = appServer.nodesPool.componentNodes
...
export const executeDocStoreUpsert = async ({
    ...
    for (const item of totalItems) {
        const res = await upsertDocStore(appDataSource, componentNodes, telemetry, storeId, item, files, isRefreshAPI, orgData)
        results.push(res)
    }
...
const refreshDocStoreMiddleware = async (storeId: string, data: IDocumentStoreRefreshData, orgData: ICommonObject) => {
    const appServer = getRunningExpressApp()
    const componentNodes = appServer.nodesPool.componentNodes
    const appDataSource = appServer.AppDataSource
    const telemetry = appServer.telemetry
