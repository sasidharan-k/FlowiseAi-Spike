...
import { constructGraphs, getAppVersion, getEndingNodes, getOrgData, getTelemetryFlowObj, isFlowValidForStream } from '../../utils'
...
const deleteChatflow = async (chatflowId: string, workspaceId?: string): Promise<any> => {
    try {
        const appServer = getRunningExpressApp()
        ...
        try {
            ...
            const { totalSize } = await removeFolderFromStorage(orgData?.id, chatflowId)
...
const getAllChatflows = async (type?: ChatflowType, workspaceId?: string): Promise<ChatFlow[]> => {
    try {
        ...
        if (type === 'MULTIAGENT') {
            ...
            return dbResponse.filter((chatflow) => chatflow.type === 'ASSISTANT' || !chatflow.type)
        }
