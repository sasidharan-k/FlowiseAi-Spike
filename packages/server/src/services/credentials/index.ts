...
import workspaceService from '../../enterprise/services/workspace'
...
const getAllCredentials = async (paramCredentialName: any, workspaceId?: string) => {
    try {
        ...
        if (paramCredentialName) {
            ...
            if (workspaceId) {
                const sharedItems: any = await workspaceService.getSharedItemsForWorkspace(workspaceId, 'credential')
