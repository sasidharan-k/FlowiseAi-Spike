...
const processLoader = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const orgData = await getOrgData(getRunningExpressApp())
        const docLoaderId = req.params.loaderId
        const body = req.body
        const apiResponse = await documentStoreService.processLoaderMiddleware(body, docLoaderId, orgData)
        return res.json(apiResponse)
    } catch (error) {
        next(error)
    }
...
const previewFileChunks = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const body = req.body
        body.preview = true
        const apiResponse = await documentStoreService.previewChunksMiddleware(body, orgData)
        return res.json(apiResponse)
    } catch (error) {
        next(error)
    }
...
const insertIntoVectorStore = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const orgData = await getOrgData(getRunningExpressApp())
        const body = req.body
        const apiResponse = await documentStoreService.insertIntoVectorStoreMiddleware(body, false, orgData)
...
const upsertDocStoreMiddleware = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const apiResponse = await documentStoreService.upsertDocStoreMiddleware(req.params.id, body, files, orgData)
        getRunningExpressApp().metricsProvider?.incrementCounter(FLOWISE_METRIC_COUNTERS.VECTORSTORE_UPSERT, {
            status: FLOWISE_COUNTER_STATUS.SUCCESS
        })
        return res.json(apiResponse)
...
const refreshDocStoreMiddleware = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const orgData = await getOrgData(getRunningExpressApp())
        const body = req.body
        const apiResponse = await documentStoreService.refreshDocStoreMiddleware(req.params.id, body, orgData)
        getRunningExpressApp().metricsProvider?.incrementCounter(FLOWISE_METRIC_COUNTERS.VECTORSTORE_UPSERT, {
            status: FLOWISE_COUNTER_STATUS.SUCCESS
        })
        return res.json(apiResponse)
