...
import { aMonthAgo, clearSessionMemory, getOrgDataFromSubdomain } from '../../utils'
...
const removeAllChatMessages = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!chatId) {
            ...
            const apiResponse = await chatMessagesService.removeChatMessagesByMessageIds(chatflowid, chatIdMap, messageIds, orgData)
            ...
            const apiResponse = await chatMessagesService.removeAllChatMessages(chatId, chatflowid, deleteOptions, orgData)
            return res.json(apiResponse)
        }
    } catch (error) {
        next(error)
