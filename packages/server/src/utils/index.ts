...
import * as AppServer from '../index'
import {
    IChatFlow,
...
export const getSubdomainFromHost = (host: string) => {
    return host?.startsWith('localhost')
        ? (process.env.DEFAULT_ORG_SUBDOMAIN as string)
        : host?.replace(process.env.HOST_DOMAIN as string, '')
}

export const getOrgData = async (appServer: AppServer.App) => {
    let orgData: ICommonObject = {}
    if (appServer.identityManager.isCloud()) {
        const supabase = appServer.app.locals.supabase
        const jwtPayload = appServer.app.locals.jwtPayload

        let orgId = ''
        if (jwtPayload && jwtPayload?.user?.user_metadata?.subdomain) {
            orgId = jwtPayload.user.user_metadata.orgId
        } else {
            const {
                data: { user },
                error: authError
        ...
        const { data: organizationData, error: orgError } = await supabase
            .from('organization')
            .select('id,subdomain,usage')
            .eq('id', orgId)
            .single()
        if (orgError) return orgData

        orgData = { id: orgId, subdomain: organizationData?.subdomain, usage: organizationData?.usage }
    } else {
        orgData = { id: await getUserInstanceId() }
    }

    return orgData
}

export const getOrgDataFromSubdomain = async (appServer: AppServer.App, host: string) => {
    let orgData: ICommonObject = {}
    let subdomain
    if (appServer.identityManager.isCloud()) {
        const supabase = appServer.app.locals.supabase
        ...
        const { data: organizationData } = await supabase
            ...
            .single()
        if (organizationData) orgData = organizationData
    } else {
        orgData = { id: await getUserInstanceId() }
    }
