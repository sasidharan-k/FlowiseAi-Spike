...
export class Telemetry {
    ...
    constructor() {
        if (process.env.IS_CLOUD === 'true') {
            this.postHog = new PostHog('phc_Icz83UXNl6vFyjTUvNpOYNLSDh3ZygQevEidLq1UCPw')
        } else {
    ...
    async sendTelemetry(event: string, properties = {}, orgId = ''): Promise<void> {
        if (this.postHog) {
            const distinctId = process.env.IS_CLOUD === 'true' ? orgId : await this.id()
