...
export const executeFlow = async ({
    ...
    if (incomingInput.uploads) {
        ...
        for (let i = 0; i < fileUploads.length; i += 1) {
            ...
            if ((upload.type === 'file' || upload.type === 'file:rag' || upload.type === 'audio') && upload.data) {
                ...
                const { totalSize } = await addSingleFileToStorage(mime, bf, filename, orgData.id, chatflowid, chatId)
            ...
            if (upload.mime === 'audio/webm' || upload.mime === 'audio/mp4' || upload.mime === 'audio/ogg') {
                logger.debug(`[server]: [${orgData?.id}]: Attempting a speech to text conversion...`)
                ...
                if (speechToTextConfig) {
                    ...
                    logger.debug(`[server]: [${orgData?.id}]: Speech to text result: ${speechToTextResult}`)
    ...
    logger.debug(`[server]: [${orgData?.id}]: Start building flow ${chatflowid}`)
    ...
    if (isAgentFlow) {
        ...
        const runParams = {
            ...
            evaluationRunId,
            storageUsage: orgData?.usage?.storage,
            updateStorageUsage,
            checkStorage,
            subdomain
        }
        ...
        logger.debug(`[server]: [${orgData?.id}]: Finished running ${endingNodeData.label} (${endingNodeData.id})`)
...
export const utilBuildChatflow = async (req: Request, isInternal: boolean = false): Promise<any> => {
    ...
    const { orgData, subdomain } = await getOrgDataFromSubdomain(appServer, req.headers.host as string)
